							...
							)
{
	...
	const int cx = get_local_size(0) * get_group_id(0) + NLM_WINDOW_RADIUS;
	const int cy = get_local_size(1) * get_group_id(1) + NLM_WINDOW_RADIUS;

	if (ix < *width && iy < *height){
		float weight = 0;

		for (int n = -NLM_BLOCK_RADIUS; n <= NLM_BLOCK_RADIUS + 1; n++){
			for (int m = -NLM_BLOCK_RADIUS; m <= NLM_BLOCK_RADIUS + 1; m++){
				weight += vecLen(
		...
		for (int i = -NLM_WINDOW_RADIUS; i <= NLM_WINDOW_RADIUS + 1; i++){
			for (int j = -NLM_WINDOW_RADIUS; j <= NLM_WINDOW_RADIUS + 1; j++){
